    ...
    Queue<TToken *> postfix(){
        ...
        while (!it.is_null()){
            ...
            if (type == TOKENSTR){
                ...
                if (!stack.empty() && stack.top()->getType() == LPAREN){
                    stack.pop(); 
                    lparen = true;
                }
                if (!lparen){
                    throw invalid_argument("Missing left parenthesis");
                } 
            } 
            else{
        ...
        while (!stack.empty()){
            TToken *popped = stack.pop();
            if (popped->getType() == LPAREN){
                throw invalid_argument("Missing right parenthesis");
            }
            postfix.push(popped);
        }
        return postfix;
