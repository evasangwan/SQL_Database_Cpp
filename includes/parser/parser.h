...
class Parser{
            ...
            switch (col){
                ...
                default:
                    if (table_name){
                        ...
                        validate_comma(token,"col","Expected: field name");
                        ptree["col"] += token;
                    } 
                    else if (fields){
                        validate_comma(token,"fields", "Expected: field name");
                        ptree["fields"] += token;
                    } 
                    else if (condition){
                        ...
                        validate_comma(token,"values", "Expected: value");
                        ptree["values"] += token;
                    } 
                    else{
   ...
   bool validate(){  //error handling 
        if (ptree.empty()){
            throw invalid_argument("Invalid command. Try again: ");
        }
        if (ptree.contains("command") && ptree["command"][0] == "select"){
            if (ptree.contains("fields")){
                if (ptree.contains("from")){
                    if (ptree.contains("table_name")){
                        if (!ptree["table_name"][0].empty() && (ptree["fields"][0] == "*" || !ptree["fields"].empty())){
                            if (ptree.contains("where")){
                                if (ptree.contains("condition") && !ptree["condition"].empty()){
                                    ...
                                    ptree.clear();
                                    throw invalid_argument("Expected: condition");
                                }
                            }
                                ...
                                if (!ptree.contains("condition")){
                                    ...
                                    ptree.clear();
                                    throw invalid_argument("Expected: where");
                                }
                            }
                        }
                    }
                        ...
                        ptree.clear();
                        throw invalid_argument("Expected: table name");
                    }
                }
                else{
                    ptree.clear();
                    throw invalid_argument("Expected: from");
                }
            }
            else{
                ptree.clear();
                throw invalid_argument("Expected: field name");
            }
        }
        else if (ptree.contains("command") && ptree["command"][0] == "make" || ptree["command"][0] == "create"){
            if (ptree.contains("table")){
                if (ptree.contains("table_name")){
                    if (ptree.contains("col")){
                        ...
                        ptree.clear();
                        throw invalid_argument("Expected: fields");
                    }
                }
                else{
                    ptree.clear();
                ...
                ptree.clear();
                throw invalid_argument("Expected: table");
            }
        }
        else if (ptree.contains("command") && ptree["command"][0] == "insert"){
            if (ptree.contains("into")){
                if (ptree.contains("table_name")){
                    if (ptree.contains("values")){
                        ...
                        ptree.clear();
                        throw invalid_argument("Expected: values");
                    }
                }
                else{
                    ptree.clear();
                ...
                ptree.clear();
                throw invalid_argument("Expected: into");
            }
        }
        ptree.clear();
